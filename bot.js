...
function makeTicketName(username) {
  const safe = username.toLowerCase().replace(/[^a-z0-9]/g,'-').slice(0,20);
  return "ticket-" + safe;
}
...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if(interaction.customId === "ticket"){
    ...
    if(interaction.channel.id !== TICKET_CHANNEL){
      return interaction.reply({content:"Use the ticket button in the ticket channel.",ephemeral:true});
    }
